...
Engine::Object::Object()
	: shader(ProjectDirectory "/Resource/Shader/Object.vert", ProjectDirectory "/Resource/Shader/Object.frag"),
	vao(), vbo(objectVertices, sizeof(objectVertices)), ebo(objectIndices, sizeof(objectIndices)),
	texture(), specularMap(),
	textures(texture.Initialize(textureImages, "Diffuse", 0, GL_RGBA, GL_UNSIGNED_BYTE)),
	specularMaps(specularMap.Initialize(specularImages, "Specular", 1, GL_RED, GL_UNSIGNED_BYTE))
{
	vao.Bind();
	vbo.Bind();
	ebo.Bind();
...
void Engine::Object::Update()
{
	...
	for (size_t i = 0; i < sizeof(textureImages) / sizeof(std::string); i++)
	{
		...
		glDrawElements(GL_TRIANGLES, sizeof(objectIndices) / sizeof(GLuint), GL_UNSIGNED_INT, (void*)(i * 6 * sizeof(GLuint)));
	}
}
